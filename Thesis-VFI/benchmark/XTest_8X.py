...
'''==========Model setting=========='''
TTA = False # Disable TTA for faster evaluation during baseline
...
for strMode in ['XTEST-2k', 'XTEST-4k']:
    ...
    for intFrame in tqdm(listFiles):
        ...
        tenEstimate = torch.clamp(tenEstimate, 0, 1)
        # Quantize to 8-bit once (consistent with saved PNG comparison)
        tenEstimate = torch.round(tenEstimate * 255.0) / 255.0
